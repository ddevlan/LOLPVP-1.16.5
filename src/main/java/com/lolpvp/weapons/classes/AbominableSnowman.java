...
public class AbominableSnowman extends BallerItem
{
	...
	private final HashSet<String> froze;
					...
					if (player.isSneaking())
					{
						if (this.lol.intValue() <= 10)
						{
							if ((Cooldowns.tryCooldown(player, "snow", 5 * 1000)) || (AbominableSnowman.this.r.contains(player.getName())))
							{
								...
								BarAPI.setMessage(player, ChatColor.AQUA + "" + ChatColor.BOLD + "Blind Radius " + this.lol.floatValue(), f);
								this.lol = this.lol.intValue() + 1;
							}
							else
							{
								AbominableSnowman.this.r.remove(player.getName());
								Long lol = Cooldowns.getCooldown(player, "snow");
								int bbb = lol.intValue() / 1000;
								player.sendMessage(ChatColor.AQUA + "Abominable Snowman" + ChatColor.RED + " is on cooldown for " + bbb);
								cancel();	
							}
						}
						else
