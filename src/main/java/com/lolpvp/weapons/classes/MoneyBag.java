					    			...
					    			for (String option : plugin.getCustomConfig().getConfigurationSection("Main.Items.").getKeys(false)) 
						    		{
					    				if(!this.isOnCooldown(p))
					    				{
					    					if (option != null) 
							    			{
							    				...
							    				double percentage = Double.parseDouble(Objects.requireNonNull(path.getString("Percentage")));
							    				if (path.get(".Name") != null && path.get(".Commands") != null)
							    				{
							    					String prize = path.getString("Name");
