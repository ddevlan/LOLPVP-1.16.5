...
public class FireWorkBow extends BallerItem
{
	...
	{
		Long hashMapID = new Random().nextLong();
		List<FireworkEffect.Type> type = new ArrayList<FireworkEffect.Type>();
		List<Color> colour = new ArrayList<Color>();
		Collections.addAll(type, FireworkEffect.Type.BALL, FireworkEffect.Type.BALL_LARGE, FireworkEffect.Type.BURST, FireworkEffect.Type.STAR, FireworkEffect.Type.CREEPER);
		Collections.addAll(colour, Color.AQUA, Color.BLACK, Color.BLUE, Color.FUCHSIA, Color.GRAY, Color.GREEN, Color.LIME, Color.MAROON, Color.NAVY, Color.OLIVE, Color.ORANGE, Color.PURPLE, Color.RED, Color.SILVER, Color.TEAL, Color.WHITE, Color.YELLOW);
		this.fireworkTimer.put(hashMapID, System.currentTimeMillis());
		Firework firework = player.getWorld().spawn(location, Firework.class);
		FireworkMeta data = firework.getFireworkMeta();
		data.addEffects(FireworkEffect.builder().withColor(colour.get(new Random().nextInt(17))).withColor(colour.get(new Random().nextInt(17))).withColor(colour.get(new Random().nextInt(17))).with(type.get(new Random().nextInt(5))).trail(new Random().nextBoolean()).flicker(new Random().nextBoolean()).build());
		data.setPower(new Random().nextInt(2) + 2);
		firework.setFireworkMeta(data);
		if (System.currentTimeMillis() - this.fireworkTimer.get(hashMapID).longValue() > 10000L) {
