...
{	
	...
	public void onLand(ProjectileHitEvent event)
	{
		if(event.getEntity() instanceof Egg)
		{
			if(this.eggs.remove(event.getEntity()))
			{
				...
				{
					event.getEntity().getWorld().playSound(egg.getLocation(), Sound.ENTITY_GENERIC_EXPLODE, 1F, 1F);
	...
	public void onHit(EntityDamageByEntityEvent event)
	{
		...
		{
			if(this.eggs.remove(event.getDamager()))
			{
