				...
				if((Core.isPlayerInPVP(player) && Core.isEntityInPVP(rightClicked)) && !InvisRing.in.contains(player.getName()))
				{
					if(Cooldowns.tryCooldown(player, "thor", 7 * 1000))
					{
						if(rightClicked instanceof Player)
						{
							...
							{
								if(!strikeTime.contains(rightClicked.getName()))
								{
									strikeTime.add(rightClicked.getName());
									new BukkitRunnable()
									{
										@Override
										public void run()
										{
											strikeTime.remove(rightClicked.getName());
										}
									}.runTaskLater(plugin, 7 * 20);
									Damageable entity2 = rightClicked;
					...
					{
						Long lol = Cooldowns.getCooldown(event.getPlayer(), "thor");
