					...
					if (player.isSneaking()) {
						if (this.lol.intValue() <= 10) {
							if ((Cooldowns.tryCooldown(player, "ice", 5 * 1000))  || (IceBlade.this.r.contains(player.getName()))) {
								...
								BarAPI.setMessage(player, ChatColor.AQUA + "" + ChatColor.BOLD + "Freeze Radius: " + this.lol.floatValue(), f);
								this.lol = this.lol.intValue() + 1;
								player.sendMessage("Time: " + lol.intValue());
							} else {
								//remove
								IceBlade.this.r.remove(player.getName());
								Long lol = Cooldowns.getCooldown(player, "ice");
								int bbb = lol.intValue() / 1000;
								player.sendMessage(IceBlade.this.getName() + ChatColor.RED + " is on cooldown for " + bbb);
								cancel();	
							}
						} else {
							//remove
			...
			if ((p instanceof Player))
			{
				...
				if(Core.isPlayerInPVP(player) && !InvisRing.in.contains(player.getName()))
				{
					if (!froze.contains(oo.getName()))
					{
						...
						ParticleEffect.CLOUD.display(oo.getLocation());
						ParticleEffect.CLOUD.display(oo.getLocation());
