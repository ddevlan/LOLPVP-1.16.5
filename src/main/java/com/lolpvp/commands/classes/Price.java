		...
		if(sender instanceof Player)
		{
			...
			if(args.length == 0)
			{
				if(ItemManager.getInstance().getBallerItemName(player.getItemInHand()) != null)
				{
					...
					player.sendMessage(ChatColor.GRAY + "The price for a " + name + ChatColor.GRAY + " is $" + ChatColor.AQUA + NumberFormat.getNumberInstance(Locale.US).format(price).replaceAll(",", ChatColor.GRAY + "," + ChatColor.AQUA) + ChatColor.GRAY + ".");
				}
				else
				{
