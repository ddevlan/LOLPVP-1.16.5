		...
		if(sender instanceof Player)
		{
			...
			if(args.length == 0)
			{
				if(ItemManager.getInstance().getBallerItemName(player.getItemInHand()) != null)
				{
					...
					Core.getEconomy().depositPlayer(player, price);
					player.setItemInHand(null);
					player.sendMessage(ChatColor.GRAY + "You have sold a " + item + ChatColor.GRAY + " for $" + ChatColor.AQUA + NumberFormat.getNumberInstance(Locale.US).format(price).replaceAll(",", ChatColor.GRAY + "," + ChatColor.AQUA) + ChatColor.GRAY + ".");
				}
				else
				{
