				...
				((sender instanceof Player)))
		{
			...
			if (player.hasPermission("lolpvp.who"))
			{
				if (args.length >= 0)
				{
					...
					for (String s : help) {
						player.sendMessage(ChatColor.translateAlternateColorCodes('&', s).replace("{ONLINE}", Integer.toString(i)).replace("{STAFF_ONLINE}", Integer.toString(b)).replace("{ALL_PLAYERS}", sb.substring(0, sb.length() - 2)).replace("{MAX}", Integer.toString(l)).replace("{DONORS}", Integer.toString(k)));
					}
				}
			}
			else {
